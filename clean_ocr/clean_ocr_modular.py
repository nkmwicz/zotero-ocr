...
def build_custom_pipeline(text, steps, verbose=False):
    """
    Build a custom cleaning pipeline from individual steps.

    Args:
        text (str): Raw PDF text
        steps (list): List of cleaning steps to apply in order
        verbose (bool): Print processing info

    Available steps:
        - "artifacts": remove_ocr_artifacts
        - "academic_metadata": remove_academic_metadata
        - "book_metadata": remove_book_metadata
        - "french_concatenation": repair_french_concatenation
        - "english_concatenation": repair_english_concatenation
        - "line_breaks": fix_line_breaks

    Returns:
        str: Cleaned text using custom pipeline

    Example:
        # Ultra-fast cleaning for speed
        clean_text = build_custom_pipeline(text, ["artifacts"])

        # Academic cleaning without concatenation repair
        clean_text = build_custom_pipeline(text, ["artifacts", "academic_metadata", "line_breaks"])

        # Book cleaning with only French concatenation
        clean_text = build_custom_pipeline(text, ["artifacts", "book_metadata", "french_concatenation", "line_breaks"])
    """
    ...
    for step in steps:
        if step in step_functions:
            text = step_functions[step](text, verbose)
        else:
            if verbose:
...
def benchmark_pipeline_performance(text, iterations=3):
    """
    Benchmark all pipeline methods for performance comparison.
    ...
    Returns:
        dict: Performance results for all methods
    """
    import time
    ...
    pipelines = {
        "fast": clean_ocr_fast,
        "academic": clean_ocr_academic,
        "book": clean_ocr_book,
        "auto": clean_ocr_auto,
    }

    results = {"text_length": len(text), "pipelines": {}}
    ...
    for name, pipeline_func in pipelines.items():
        ...
        for i in range(iterations):
            start = time.time()
            result_text = pipeline_func(text, verbose=False)
            end = time.time()
    ...
    # Calculate speedup factors relative to auto pipeline
    auto_time = results["pipelines"]["auto"]["avg_time"]
    for name in results["pipelines"]:
        pipeline_time = results["pipelines"][name]["avg_time"]
        results["pipelines"][name]["speedup_vs_auto"] = (
            auto_time / pipeline_time if pipeline_time > 0 else 0
        )

    return results


def test_individual_components(text, verbose=True):
    """
    Test each cleaning component individually to identify performance bottlenecks.
    ...
    Returns:
        dict: Performance results for each component
    """
    import time

    components = {
        "OCR Artifacts": remove_ocr_artifacts,
        "Academic Metadata": remove_academic_metadata,
        "Book Metadata": remove_book_metadata,
        "French Concatenation": repair_french_concatenation,
        "English Concatenation": repair_english_concatenation,
        "Line Breaks": fix_line_breaks,
    }
    ...
    for name, func in components.items():
        start = time.time()
        result = func(text, verbose=False)
        end = time.time()
        ...
        results[name] = {
            ...
            "change": len(result) - len(text),
        }

        if verbose:
            change_pct = (results[name]["change"] / len(text)) * 100
            print(
                f"{name:20} {elapsed:6.3f}s  {len(text):>7,} → {len(result):>7,} chars ({change_pct:+5.1f}%)"
            )
